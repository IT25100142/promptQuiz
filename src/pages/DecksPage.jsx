  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100">My Study Library</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your active recall card decks and study quizzes.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-xl border border-slate-300 dark:border-slate-755 bg-white dark:bg-slate-850 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors"
          >
            Export library
          </button>
          <button
            ...
            disabled={deckLoading}
            className="inline-flex items-center justify-center rounded-xl border border-amber-300 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-955/30 px-4 py-2.5 text-sm font-semibold text-amber-800 dark:text-amber-300 shadow-sm hover:bg-amber-100 dark:hover:bg-amber-900/20 disabled:opacity-50 transition-colors"
          >
            Import library
          </button>
          <button
      ...
      {hasDecks ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {library.savedDecks.map(deck => {
            ...
            return (
              <div 
                key={deck.id} 
                className="flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 overflow-hidden"
              >
                {/* Header card details */}
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{deck.name}</h2>
                    <button
                      type="button"
                      onClick={() => handleDeleteDeck(deck.id)}
                      className="text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 transition-colors text-xs font-semibold p-1"
                  ...
                  <span className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-6">
                    Created: {new Date(deck.date).toLocaleDateString()}
                  </span>

                  {/* Quizzes Sub-list */}
                  <div className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Quizzes</h3>
                    {quizzes.length > 0 ? (
                        ...
                        {quizzes.map(quiz => {
                          ...
                          return (
                            <button
                              ...
                              className="w-full flex items-center justify-between text-left p-2.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-850 hover:border-indigo-200 dark:hover:border-indigo-850 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition group"
                            >
                              <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 group-hover:text-indigo-900 dark:group-hover:text-indigo-200 transition-colors truncate pr-2">
                                📝 {quiz.name}
                              </span>
                              <span className="shrink-0 bg-indigo-100 dark:bg-indigo-950/55 text-indigo-800 dark:text-indigo-300 text-[10px] font-extrabold px-2 py-0.5 rounded-full">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/60 p-4 border-t border-slate-100 dark:border-slate-800">
                  {activeDeckForNewQuiz === deck.id ? (
                    <div className="flex flex-col gap-2">
                      <input
                        ...
                        className="w-full px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-750 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      />
                    ...
                    <button
                      ...
                      className="w-full text-center text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 py-1"
                    >
                      + Add New Quiz
                    </button>
                  )}
        ...
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center bg-white dark:bg-slate-900 shadow-sm mt-4">
          <span className="text-5xl mb-4 animate-bounce">📚</span>
          <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100">Your Library is Empty</h2>
